...
public class GuardiaRobot extends GenericRobot implements Client {
	...
	public void explore()
	{
		...
		{	
			...
			{
				//System.out.println(goal + " giÃ  esaminato");
				continue;
			}
			...
			if (path == null)
			{
				//System.out.println("Non c'Ã¨ un path per " + goal);
				updateMapAndSendPacket(goal);
				closedSet.add(new Point(goal));
				continue;
			}
			
			//System.out.println("C'Ã¨ un path per " + goal);
			...
			for (int i = 0; i < correctedPath.size(); ++i)
			{
				...
				{	
					...
					if (path == null)
					{
						//System.out.println("Non ho piÃ¹ un path per " + goal);
						updateMapAndSendPacket(goal);
						closedSet.add(new Point(goal));
